...
def test_source_specific_external_ids() -> None:
    ...
    assert build_external_id("github_releases", {"repo_full_name": "openai/openai-python", "tag_name": "v2"}) == "openai/openai-python:v2"
    assert build_external_id("arxiv", {"id": "https://arxiv.org/abs/2601.00001"}) == "2601.00001"
    assert build_external_id("hugging_face", {"modelId": "openai/example-model"}) == "openai/example-model"
    assert build_external_id("npm", {"name": "ai-sdk", "version": "1.0.0"}) == "ai-sdk:1.0.0"
    assert build_external_id("official_blog", {"guid": "post-1"}) == "post-1"
