...
import { getTrendById, trends } from "@/src/mockData";

export function generateStaticParams() {
  return trends.map((trend) => ({ entity_id: trend.id }));
}

export default function TrendDetailPage({ params }: { params: { entity_id: string } }) {
  const trend = getTrendById(params.entity_id);
    ...
    <div className="space-y-6">
      <section className="border-b border-border pb-5">
        <div className="flex flex-wrap items-center gap-2">
          <h1 className="text-3xl font-semibold tracking-normal">{trend.canonicalName}</h1>
          <ScoreBadge score={trend.score.finalScore} verdict={trend.score.verdict} />
        </div>
        <p className="mt-2 max-w-3xl text-sm text-muted-foreground">{trend.summary}</p>
      </section>

      <section className="grid gap-4 lg:grid-cols-[1fr_360px]">
        <div className="rounded-md border border-border bg-card p-4">
          <h2 className="text-sm font-semibold uppercase tracking-wide text-muted-foreground">Source Breakdown</h2>
        ...
        <div className="rounded-md border border-border bg-card p-4">
          <h2 className="text-sm font-semibold uppercase tracking-wide text-muted-foreground">Score Components</h2>
      ...
      <section className="rounded-md border border-border bg-card p-4">
        <h2 className="text-sm font-semibold uppercase tracking-wide text-muted-foreground">Mentions</h2>
        <div className="mt-4 divide-y divide-border">
